/* Doomroom News — app.js (FETCH VERSION)
   Uses GDELT DOC API with normal fetch() (no JSONP).
*/
...
async function fetchJson(url, timeoutMs = 12000) {
  ...
  try {
    ...
    // If CORS blocks, this usually throws earlier — but if it doesn't:
    if (!res.ok) {
...
async function fetchDriver(query, timespan = "6h", maxrecords = 60) {
  ...
  // GDELT sometimes returns errors in a JSON envelope
...
async function refresh() {
  ...
  try {
    const pulls = await Promise.all(
      DRIVERS.map((q) => fetchDriver(q, "6h", 60).catch((e) => {
        // swallow per-driver, but keep a breadcrumb for debugging
        console.warn("Driver failed:", q, e);
        return [];
      }))
    );
    ...
    // If we got nothing, show a clear error (instead of silently “0”)
    if (!all.length) {
      throw new Error("No articles returned from GDELT. (Fetch blocked, API down, or query empty.)");
    }
