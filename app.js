/* Doomroom News — GitHub Pages safe build (ONE-request version)
   Fix: proxies often return HTML error pages ("<...") + rate-limit when you do many requests.
   Solution: do ONE GDELT request, then categorize locally.
*/
...
// GDELT endpoints (we’ll try both)
const GDELT_BASES = [
  "https://api.gdeltproject.org/api/v2/doc/doc",
  "https://i.gdeltproject.org/api/v2/doc/doc",
];

// CORS helpers (public, flaky, but with ONE request it usually works)
const PROXIES = [
  // tends to work often
  (url) => `https://api.allorigins.win/raw?url=${encodeURIComponent(url)}`,
  // sometimes works when allorigins doesn’t
  (url) => `https://corsproxy.io/?${encodeURIComponent(url)}`,
  // another fallback
  (url) => `https://thingproxy.freeboard.io/fetch/${url}`,
  // text proxy fallback (returns plain text; sometimes adds extra content)
  (url) => `https://r.jina.ai/http://${url.replace(/^https?:\/\//, "")}`,
];

// Broad query (we pull a general “doom-ish” feed, then classify locally)
const BROAD_QUERY = [
  "war", "missile", "drone", "invasion", "ceasefire", "strike",
  "climate", "heatwave", "wildfire", "flood", "drought", "storm",
  "recession", "inflation", "layoffs", "rates", "bank", "debt", "gdp",
  "election", "coup", "protest", "crackdown", "corruption", "fraud",
  "hack", "ransomware", "breach", "malware", "leak", "ddos",
  "nuclear", "radiation", "uranium", "reactor", "warhead",
  "asteroid", "meteor", "comet", "rocket", "satellite", "lunar", "mars",
].map(w => `"${w}"`).join(" OR ");
...
function safeJSONParse(text) {
  ...
  // If it starts like HTML, it’s not JSON.
  if (t.startsWith("<") || t.toLowerCase().includes("<html")) {
    throw new Error("Proxy returned HTML (blocked/rate-limited).");
  }

  // Sometimes proxies add anti-XSSI prefix
  const cleaned = t.replace(/^\)\]\}',?\s*/, "");
  return JSON.parse(cleaned);
}

async function fetchTextWithTimeout(url, timeoutMs = 14000) {
...
async function fetchJSONWithFallbacks(url) {
  // Try direct first
  try {
    const txt = await fetchTextWithTimeout(url, 9000);
  ...
  for (const makeProxy of PROXIES) {
    ...
    try {
      const txt = await fetchTextWithTimeout(proxied, 16000);
      return safeJSONParse(txt);
    } catch (e) {
  ...
  throw new Error(`All fetch attempts failed. Last errors: ${errs.slice(-3).join(" | ")}`);
}
...
async function fetchGdeltOnce() {
  ...
  for (const base of GDELT_BASES) {
    const url = buildGdeltUrl(base, BROAD_QUERY, DEFAULT_TIMESPAN, MAXRECORDS);
    try {
...
function classifyTitle(title) {
  ...
  // Weight: more tags = more “doom overlap”
...
function renderLists(result) {
  ...
  for (const d of result.drivers) {
    ...
    item.className = "item";
    item.innerHTML = `<div class="item__title">${driverName(d.k)}</div><div class="item__meta">${d.v} hits</div>`;
    els.driversList.appendChild(item);
  }

  result.stories.slice(0, 40).forEach((s) => els.storiesList.appendChild(storyCard(s)));
}
...
async function refresh() {
  try {
    ...
    const raw = await fetchGdeltOnce();
    const all = dedupeArticles(raw).slice(0, 140);

    if (all.length === 0) {
      els.doomValue.textContent = "!!";
      els.doomLabel.textContent = "Error loading headlines.";
      setStatus("No articles returned (empty response).", true);
      return;
    }
    ...
    // Show the error in the stories list so you can screenshot it
    const warn = document.createElement("div");
    warn.className = "item";
    warn.innerHTML =
      `<div class="item__title">Fetch failed.</div>` +
      `<div class="item__meta">${VERSION}</div>` +
      `<div class="item__satire">${String(err?.message || err)}</div>`;
